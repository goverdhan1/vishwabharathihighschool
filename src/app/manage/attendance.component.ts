...
//    this[formName].controls['gamount'].setValue(this.total_amount.toFixed(2));
  ...
  getData(formData?) {
    ...
    this.querySubscription = this._backendService.getDocs('ATTENDANCE', formData).subscribe((res) => {
  ...
  getDoc(docId) {
    this.dataLoading = true;
    this.data$ = this._backendService.getDoc('ATTENDANCE', docId).subscribe(res => {
      if (res) {
        this.data$ = res;
        this.editDataForm = this._fb.group({
          _id: ['', Validators.required],
