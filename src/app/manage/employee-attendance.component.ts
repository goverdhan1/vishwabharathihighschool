...
export class EmployeeAttendanceComponent implements OnInit, OnDestroy, AfterViewInit {
  members: any[];
  ...
  displayedColumns: Array<any> = ['code', 'lName', 'fName', 'designation'];
  editable = false;
  ...
  submitAttendance(event, docId, field) {
    ...
    this.querySubscriptionSpecific = this.attendanceService.updateSpecificDayEmpAttendance( this.enrollId, docId, data).then((res: any) => {
      console.log(res);
        }).catch(err => {
          if (err) {
              this.error = true;
...
//    this.ref.markForCheck();
  ...
  getData(enrollId: string, formData?) {
    this.dataLoading = true;
    this.querySubscriptionList = this.attendanceService.getEmployeeAttendanceList(enrollId).subscribe((res: any) => {
      console.log(res);
      ...
      this.displayedColumns = ['code', 'lName', 'fName', 'designation'];
      this.days = [];
