...
export class AttendanceService {
  ...
  getAttendanceList(enrollId: string) {
    return combineLatest(
      ...
      this._afs.collection('SMS_CONFIG_ENROLL_CD').doc(enrollId).collection('ATTENDANCE').valueChanges()
...
// });
  ...
  getUserStudentAttendanceDoc(enrollId) {
   ...
   .pipe(switchMap(res => this._afs.collection('SMS_CONFIG_ENROLL_CD').doc(enrollId).collection('STUDENTS',
    ref => ref.where('SKEY', '==', res['phone'])).valueChanges()
   .pipe(switchMap(
     res => this._afs.collection('SMS_CONFIG_ENROLL_CD').doc(enrollId).collection('ATTENDANCE').valueChanges()))
    ));
  }
  ...
  updateSpecificDayAttendance(enrollId: string, docId: string, data: any) {
    const docRef = this._afs.collection('SMS_CONFIG_ENROLL_CD').doc(enrollId).collection('ATTENDANCE').doc(docId);
